---
source_file: "tests/integration/conftest.py"
type: "code"
community: "Architecture Decision Records"
location: "clickhouse_schema fixture"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Architecture_Decision_Records
---

# raw.saas_events ClickHouse Table

## Connections
- [[ClickHouse Integration Tests]] - `references` [EXTRACTED]
- [[Integration Test Fixtures (conftest.py)]] - `implements` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Architecture_Decision_Records